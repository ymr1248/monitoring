        ...
        <div class="grid_3 grid_5 widget-shadow">
           <div>
            ...
            <div style="float: left;">
                <h3>
                    <a href="toOperatorAddMonitor?groupId=${groupId}"><span class="label label-success">添加监控器</span></a>
                </h3>
            </div>  
            	...
            	<c:choose>
            		<c:when test="${search!=null }">
            			<form action="toOperatorSeeMonitor1" method="post">
                    ...
                    <c:if test="${monitorList.size()>0}">
                        ...
                        <c:forEach items="${monitorList}" var="item" varStatus="status">
									<tr class="manager-bg input-lg">
										...
										<td class="manager-border2">${item.monitorStatus}</td>
										<td class="manager-border2"><a
											href="to_operator_monitor_info?id=${item.id}">
				...
				<c:when test="${search!=null }">
                    ...
                    <ul class="pagination">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <li><a href="toOperatorSeeMonitor1?pageNum=1&groupId=${groupId}&search=${search}">首页</a></li>
                                <li>
                                    <a href="toOperatorSeeMonitor1?pageNum=${pageNum-1}&groupId=${groupId}&search=${search}"
                                       aria-label="Previous">
                                        <span aria-hidden="true">«</span>
                                    </a>
                                </li>
                        ...
                        <c:forEach var="k" begin="1" end="${totalPage}">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <li>
                                        <a href="toOperatorSeeMonitor1?pageNum=${k}&groupId=${groupId}&search=${search}">${k}</a>
                                    </li>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
                        <c:choose>
                            ...
                            <c:otherwise>
                                <li>
                                    <a href="toOperatorSeeMonitor1?pageNum=${pageNum+1}&groupId=${groupId}&search=${search}" aria-label="Next">
                                        <span aria-hidden="true">»</span>
                                    </a>
                                </li>
                                <li>
                                    <a href="toOperatorSeeMonitor1?pageNum=${totalPage}&groupId=${groupId}&search=${search}">尾页</a>
                                </li>
                            </c:otherwise>
                        </c:choose>
                    </ul>
...
<script>
	 ...
	 $("#button2").click(function(){
    	if($("input:checkbox[name='id']:checked").length==0){
    		...
    		var r=confirm("确定删除所选监控器？");
    		 if (r==true){
		    	var newUrl = 'operatormonitor_deleteBoth';
	...
	$('#only').click(function(){
		var newUrl = 'operatormonitor_removeBoth';
